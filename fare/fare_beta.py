...
def query_exceptions(agency_id,route_type,route_id, c):
    '''
    ...
    c.execute("SELECT * FROM Exceptions WHERE agency_id=:ag_id AND route_type=:r_tp AND route_id=:r_id",
              {'ag_id':agency_id, 'r_tp':route_type, 'r_id':route_id})
    lst_qry = c.fetchall()
    # conn.close()
    if not lst_qry:
...
def query_agency(agency_id,agency_name,region_id,route_type, c):
    ...
    # c = conn.cursor()
    # query
    c.execute("SELECT * FROM Agency WHERE agency_id=:ag_id AND agency_name=:ag_name AND route_type=:r_tp AND fare_type_id <> 5",
              {'ag_id':agency_id, 'ag_name':agency_name,'r_tp':route_type})
    lst_qry = c.fetchall()
    # conn.close()
    if not lst_qry:
        return None
...
def query_zone_fare(agency_id,agency_name,route_type,origin_zone,destination_zone, c):
    '''
    ...
    c.execute("SELECT * FROM ZoneFare WHERE agency_id=:ag_id AND agency_name=:ag_name AND region_id=:reg_id AND route_type=:r_tp AND origin_zone=:orig AND destination_zone=:dest",
              {'ag_id':agency_id, 'ag_name':agency_name,'r_tp':route_type,'orig':origin_zone,'dest': destination_zone})
    lst_qry = c.fetchall()
    # conn.close()
    if not lst_qry:
        return None
...
def query_zone(agency_id,agency_name,route_type,stop_id, c):
    '''
    ...
    # c = conn.cursor()
    # query
    c.execute("SELECT * FROM Zone WHERE agency_id=:ag_id AND agency_name=:ag_name AND route_type=:r_tp AND stop_id=:stp_id",
              {'ag_id':agency_id,'ag_name':agency_name,'r_tp':route_type,'stp_id':stop_id})
    lst_qry = c.fetchall()
    # conn.close()
    if not lst_qry:
        return None
...
def query_static_fare(agency_id,route_type, agency_name,c):
    '''
    Search flat fares.
    ...
    c.execute("SELECT * FROM StaticFare WHERE agency_id=:ag_id AND route_type=:r_tp AND region_id=:rg_id AND agency_name=:ag_name",
              {'ag_id':agency_id,'r_tp':route_type, 'ag_name': agency_name})
    lst_qry = c.fetchall()
    # conn.close()
    if not lst_qry:
        return None
...
def query_transfer_rules(current_agency_id,current_route_type,current_route_id,current_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id,current_agency_name,previous_agency_name,c):
    '''
...
def lookup_transfer(current_agency_id,current_route_type,current_route_id,current_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id,region_id,current_agency_name,previous_agency_name,c):
    ...
    '''    
    # stop level    
    stop_level = query_transfer_rules(current_agency_id,current_route_type,current_route_id,current_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id,current_agency_name,previous_agency_name,c)
    # route level
    route_level = query_transfer_rules(current_agency_id,current_route_type,current_route_id,'.',previous_agency_id,previous_route_type,previous_route_id,'.',current_agency_name,previous_agency_name, c)
    # mode level
    mode_level = query_transfer_rules(current_agency_id,current_route_type,'.','.',previous_agency_id,previous_route_type,'.','.',current_agency_name,previous_agency_name, c)
    # agency level
...
def transfer_update(rules_id_true,leg_item,leg_duration,flag,transfer_list,current_agency_id,current_route_type,current_route_id,current_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id, current_agency_name,previous_agency_name,c):   
    '''
    Update fare rules.
    ...
    transfer_rules = lookup_transfer(current_agency_id,current_route_type,current_route_id,current_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id, current_agency_name,previous_agency_name,c)
            ...
            if exc_fare == None: 
                ...
                if fare_type_id == 1: #flat fare
                    ...
                    fare = query_zone_fare(previous_agency_id,previous_agency_name,region_id,previous_route_type,
                                    query_zone(previous_agency_id,previous_agency_name,previous_route_type,previous_stop_id, c),
                                    query_zone(previous_agency_id,previous_agency_name,previous_route_type,next_stop_id, c), c)    
                else:
                    fare = 0
            else:
        ...
        # Case: transfer != 0
            ...
            for leg_index, leg_item in enumerate(control_lst):
                ...
                if leg_index < (len(control_lst) - 1): # while is not the last leg index
                    ...
                    fare_type_id = query_agency(previous_agency_id,previous_agency_name,previous_route_type, c)
                ...
                if flag == 0:
                    ...
                    if leg_index < (len(control_lst) - 1):
                        # verify if fare is within Exception table
                        exc_fare = query_exceptions(previous_agency_id,previous_route_type,previous_route_id, c)                                         
                        if exc_fare == None:
                            # Query Agency and verify fare_type_id                           
                            fare_type_id = query_agency(previous_agency_id,previous_agency_name,previous_route_type, c)
                            # Query for each type of fare
                            if fare_type_id == 1: # flat fare
                                fare = query_static_fare(previous_agency_id,previous_route_type, previous_agency_name,c)
                                partial_cost.append(fare)
                                transfer_list, flag, cost,rule_true = transfer_update(rule_true,leg_item,leg_duration,flag,transfer_list,next_agency_id,next_route_type,next_route_id,next_stop_id,previous_agency_id,previous_route_type,previous_route_id,previous_stop_id,next_agency_name,previous_agency_name,c)
                                partial_cost.append(cost)
                            elif fare_type_id == 2: # zonal fares
                                fare = query_zone_fare(previous_agency_id,previous_agency_name,previous_route_type,
                                                query_zone(previous_agency_id,previous_agency_name,previous_route_type,previous_stop_id, c),
                                                query_zone(previous_agency_id,previous_agency_name,previous_route_type,next_stop_id, c), c)
                                partial_cost.append(fare)
